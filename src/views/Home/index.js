...
const Home = () => {
    ...
    useEffect(() => {
        dispatch({type: 'GET_CATALOG', payload: data}) //dispatch - создает новый action и отпр-ет данные в reducer.
            ...
            <div className='row'>
                {
                    catalog.map(product =>
                        <div className='col-md-3 mb-4'>
                            ...
                            <h4>{product.title}</h4>
                            <p>{product.price}</p>
                            <button onClick={() => dispatch({type: 'ADD_TO_CART', payload: product})}>В корзинку</button>
